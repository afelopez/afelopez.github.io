...
- `FeaturedProjects.tsx` also contains a `LINK_STYLES` constant that is defined but never referenced anywhere in the file's render — dead code, confirmed by reading the whole file. Not carried over into the new component. `LINK_ICONS` **is** used (renders an icon glyph per `link.type`) and **is** carried over.
...
- **Content panel**: `project.name` as heading, `project.description`, `project.tech` pills, `project.links` buttons with `LINK_ICONS[link.type]` — same content and structure as today's center column, just restyled with the teal accent (§3.6).
